...
function modularPipelineReducer() {
  let modularPipelineState = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  ...
  switch (action.type) {
    case _modularPipelines.TOGGLE_MODULAR_PIPELINE_ACTIVE:
      {
        return updateState({
          active: Object.assign({}, modularPipelineState.active, batchChanges('active'))
        });
      }
    case _modularPipelines.TOGGLE_MODULAR_PIPELINE_DISABLED:
      {
        return updateState({
          disabled: Object.assign({}, modularPipelineState.disabled, batchChanges('disabled'))
        });
      }
    case _modularPipelines.TOGGLE_SINGLE_MODULAR_PIPELINE_EXPANDED:
      {
        const newVisibleState = {
          ...modularPipelineState.visible
        };
        newVisibleState[action.modularPipelineID] = false;
        modularPipelineState.tree[action.modularPipelineID].children.forEach(child => newVisibleState[child.id] = true);
        return updateState({
    ...
    case _modularPipelines.TOGGLE_MODULAR_PIPELINES_EXPANDED:
      {
        const newVisibleState = {
          ...modularPipelineState.visible
        };
        ...
        if (isExpanding && modularPipelineState.ids.length > 0) {
          const expandedModularPipelines = expandedIDs.filter(expandedID => !modularPipelineState.expanded.includes(expandedID));
          expandedModularPipelines.forEach(expandedModularPipeline => {
            newVisibleState[expandedModularPipeline] = false;
            modularPipelineState.tree[expandedModularPipeline].children.forEach(child => newVisibleState[child.id] = true);
          });
        } else {
          const collapsedModularPipelines = modularPipelineState.expanded.filter(expandedID => !expandedIDs.includes(expandedID));
          ...
          collapsedModularPipelines.forEach(collapsedModularPipeline => {
            ...
            expandedIDs = expandedIDs.filter(id => !id.startsWith(collapsedModularPipeline));
          });
        }
        return updateState({
...
var _default = exports.default = modularPipelineReducer;