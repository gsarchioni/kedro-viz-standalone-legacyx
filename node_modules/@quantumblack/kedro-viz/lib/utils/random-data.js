...
var _randomUtils = _interopRequireWildcard(require("./random-utils"));
...
const LAYERS = ['Raw', 'Intermediate', 'Primary', 'Feature', 'Model Input', 'Model Output'];
...
class Pipeline {
  constructor() {
    this.utils = (0, _randomUtils.default)();
  ...
  generatePipelines() {
    ...
    const pipelineCount = this.utils.randomNumberBetween(MIN_PIPELINES_COUNT, MAX_PIPELINES_COUNT);
    for (let i = 1; i < pipelineCount; i++) {
      pipelines.push(this.utils.getRandomName(this.utils.randomNumber(4), ' '));
    }
    return pipelines.filter(_index.unique);
  }
   ...
   */
  getRankCount() {
    let rankCount = this.utils.randomNumberBetween(MIN_RANK_COUNT, MAX_RANK_COUNT);
   ...
   */
  getRankLayers() {
    const layerSize = (0, _index.arrayToObject)(LAYERS, () => 0);
  ...
  generateTags() {
    ...
    return (0, _randomUtils.getNumberArray)(tagCount).map(() => this.utils.getRandomName(this.utils.randomNumber(MAX_NODE_TAG_COUNT))).filter(_index.unique);
  }
   ...
   */
  getRankNodeCount() {
    return Math.min(MIN_RANK_NODE_COUNT / this.utils.random(), MAX_RANK_NODE_COUNT);
  }
  ...
  createNode(i, initialRank) {
    ...
    const node = {
      id: "".concat(layer, "_").concat(initialRank, "_").concat(i),
      name: null,
      type: null,
      rank: initialRank,
      layer: layer,
  ...
  getNodeName(type) {
    ...
    return type === 'parameters' ? "Parameters ".concat(name) : name;
  }
  ...
  getNodeMetaData(node) {
    ...
    node.parameters = (0, _index.arrayToObject)((0, _randomUtils.getNumberArray)(randomNumber(10)).map(() => getRandomName(randomNumber(2), '_')), () => this.utils.randomNumber(50) / 10);
  }
   ...
   */
  getRandomTags() {
    return this.utils.getRandomSelection(this.tags, this.utils.randomNumber(this.tags.length));
  }
  ...
  generateEdges() {
    const edges = [];
    const edgeCount = this.utils.randomNumberBetween(MIN_EDGE_COUNT, MAX_EDGE_COUNT);
    const nodesByRank = this.getNodesByRank();

    // Find the sorted list of node ranks
    const ranks = Object.keys(nodesByRank).map(rank => parseFloat(rank)).sort((a, b) => a - b);
    ...
    for (let i = 0; i < edgeCount; i += 1) {
      ...
      const targetRankIndex = sourceRankIndex + Math.min(biasedRandom, remainingRankCount);
  ...
  activeNodes() {
    ...
    for (const edge of this.edges) {
      ...
      if (degree(edge._sourceNode) + degree(edge._targetNode) > MIN_NODE_DEGREE) {
   ...
   */
  activeTags() {
    return this.nodes.reduce((tags, node) => node.tags ? tags.concat(node.tags) : tags, []).filter(_index.unique).map(tag => ({
      name: tag,
      id: tag
    }));
  }
   ...
   */
  activeEdges() {
    const visibleNodes = (0, _index.arrayToObject)(this.nodes.map(node => node.id), () => true);
    return this.edges.filter(edge => visibleNodes[edge.target] && visibleNodes[edge.source]);
  }
  ...
  update() {
    ...
    const sortedNodes = (0, _batchingToposort.default)(graph);
    for (let rank = 0; rank < sortedNodes.length; rank++) {
      for (const id of sortedNodes[rank]) {
        const node = this.nodes.find(node => node.id === id);
        node.rank = rank;
        node.type = this.getType(node);
        node.name = "".concat(node.layer, "_").concat(node.type, "_").concat(node.rank, "_").concat(this.getNodeName(node.type)).replace(/\s/g, '_');
  ...
  all() {
    return {
      ...
      layers: LAYERS,
      nodes: this.nodes,
      pipelines: this.pipelines.map(name => ({
        id: name,
        name
      })),
...
var _default = exports.default = generateRandomPipeline;