...
const layout = _ref => {
  ...
  (0, _solver.solveStrict)([...rowConstraints, ...layerConstraints], constants, 1);
  ...
  for (let i = 0; i < iterations; i += 1) {
    (0, _solver.solveLoose)(crossingConstraints, 1, constants);
    (0, _solver.solveLoose)(parallelConstraints, 50, constants);
  }
  ...
  (0, _solver.solveStrict)([...separationConstraints, ...parallelConstraints], constants, 1);
...
const createRowConstraints = edges => edges.map(edge => ({
  base: _constraints.rowConstraint,
  a: edge.targetNode,
  b: edge.sourceNode
}));
...
const createLayerConstraints = (nodes, layers) => {
  ...
  const layerGroups = layers.map(name => nodes.filter(node => node.nearestLayer === name));
  ...
  for (let i = 0; i < layerGroups.length - 1; i += 1) {
    ...
    for (const node of layerNodes) {
      layerConstraints.push({
        base: _constraints.layerConstraint,
        a: intermediary,
        b: node
      });
    }
...
const createCrossingConstraints = (edges, constants) => {
  ...
  for (let i = 0; i < edges.length; i += 1) {
    ...
    const edgeADegree = sourceA.sources.length + sourceA.targets.length + targetA.sources.length + targetA.targets.length;
    for (let j = i + 1; j < edges.length; j += 1) {
      ...
      // Count the connected edges
      const edgeBDegree = sourceB.sources.length + sourceB.targets.length + targetB.sources.length + targetB.targets.length;
      crossingConstraints.push({
        base: _constraints.crossingConstraint,
        edgeA: edgeA,
        edgeB: edgeB,
 ...
 */
const createParallelConstraints = edges => edges.map(_ref2 => {
  let {
    sourceNode,
    targetNode
  } = _ref2;
  return {
    base: _constraints.parallelConstraint,
    a: sourceNode,
    b: targetNode,
    // Evenly distribute the constraint
    strength: 0.6 / Math.max(1, sourceNode.targets.length + targetNode.sources.length - 2)
  };
});
...
const createSeparationConstraints = (rows, constants) => {
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    rowNodes.sort((a, b) => (0, _common.compare)(a.x, b.x, a.id, b.id));
    ...
    for (let j = 0; j < rowNodes.length - 1; j += 1) {
      ...
      const space = (0, _common.snap)(spread * spaceX, spaceX);
      separationConstraints.push({
        base: _constraints.separationConstraint,
        a: nodeA,
        b: nodeB,
 ...
 */
const expandDenseRows = function (edges, rows, spaceY) {
  let scale = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 1.25;
  let unit = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 0.25;
  ...
  for (let i = 0; i < rows.length - 1; i += 1) {
    ...
    const offsetY = (0, _common.snap)(density * scale * spaceY, spaceYUnit);
...
const rowDensity = edges => {
  ...
  for (const edge of edges) {
    ...
    const edgeAngle = Math.abs((0, _common.angle)(edge.targetNode, edge.sourceNode) - _common.HALF_PI) / _common.HALF_PI;
