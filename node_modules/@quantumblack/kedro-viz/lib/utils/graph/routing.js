...
const routing = _ref => {
  ...
  const rows = (0, _common.groupByRow)(nodes);

  // For each node
  for (const node of nodes) {
    // Sort the node's target edges by the angle between source and target nodes
    node.targets.sort((a, b) => (0, _common.compare)((0, _common.angle)(b.sourceNode, b.targetNode), (0, _common.angle)(a.sourceNode, a.targetNode)));
  }
  ...
  for (const edge of edges) {
    ...
    const sourceSeparation = Math.min((source.width - stemSpaceSource) / source.targets.length, stemSpaceSource);
    const sourceEdgeDistance = source.targets.indexOf(edge) - (source.targets.length - 1) * 0.5;
    ...
    for (let i = source.row + 1; i < target.row; i += 1) {
      ...
      let nearestPoint = {
        x: (0, _common.nodeLeft)(firstNode) - spaceX,
        y: firstNode.y
      };
      ...
      for (let i = 0; i < rowExtended.length - 1; i += 1) {
        ...
        const nodeGap = (0, _common.nodeLeft)(nextNode) - (0, _common.nodeRight)(node);
        ...
        const candidatePoint = (0, _common.nearestOnLine)(currentPoint.x, currentPoint.y, (0, _common.nodeRight)(node) + offsetX, (0, _common.nodeTop)(node) - spaceY, (0, _common.nodeLeft)(nextNode) - offsetX, (0, _common.nodeTop)(nextNode) - spaceY);
        const distance = (0, _common.distance1d)(currentPoint.x, candidatePoint.x);
  ...
  for (const node of nodes) {
    ...
    node.targets.sort((a, b) => (0, _common.compare)((0, _common.angle)(b.sourceNode, b.points[0] || b.targetNode), (0, _common.angle)(a.sourceNode, a.points[0] || a.targetNode)));
    // Sort the node's incoming edges by the ending angle of the edge path
    node.sources.sort((a, b) => (0, _common.compare)((0, _common.angle)(a.points[a.points.length - 1] || a.sourceNode, a.targetNode), (0, _common.angle)(b.points[b.points.length - 1] || b.sourceNode, b.targetNode)));
  }
  ...
  for (const edge of edges) {
    ...
    const sourceSeparation = Math.min((source.width - stemSpaceSource) / source.targets.length, stemSpaceSource);
    const targetSeparation = Math.min((target.width - stemSpaceTarget) / target.sources.length, stemSpaceTarget);
    const sourceEdgeDistance = source.targets.indexOf(edge) - (source.targets.length - 1) * 0.5;
    const targetEdgeDistance = target.sources.indexOf(edge) - (target.sources.length - 1) * 0.5;
    const sourceOffsetX = sourceSeparation * sourceEdgeDistance;
    const targetOffsetX = targetSeparation * targetEdgeDistance;

    // Decrease stem length outwards from the middle stem
    const sourceOffsetY = stemUnit * source.targets.length * (1 - Math.abs(sourceEdgeDistance) / source.targets.length);
    const targetOffsetY = stemUnit * target.sources.length * (1 - Math.abs(targetEdgeDistance) / target.sources.length);

    // Build the source stem for the edge
    const sourceStem = [{
      x: source.x + sourceOffsetX,
      y: (0, _common.nodeBottom)(source)
    }, {
      x: source.x + sourceOffsetX,
      y: (0, _common.nodeBottom)(source) + stemMinSource
    }, {
      x: source.x + sourceOffsetX,
      y: (0, _common.nodeBottom)(source) + stemMinSource + Math.min(sourceOffsetY, stemMax)
    }];

    // Build the target stem for the edge
    const targetStem = [{
      x: target.x + targetOffsetX,
      y: (0, _common.nodeTop)(target) - stemMinTarget - Math.min(targetOffsetY, stemMax)
    }, {
      x: target.x + targetOffsetX,
      y: (0, _common.nodeTop)(target) - stemMinTarget
    }, {
      x: target.x + targetOffsetX,
      y: (0, _common.nodeTop)(target)
    }];
