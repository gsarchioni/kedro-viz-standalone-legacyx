...
const graphNew = _ref => {
  ...
  for (const node of nodes) {
    ...
    const fullName = node && node.fullName && node.fullName.length || node && node.name && node.name.length;
    const padding = {
      x: 20,
      y: 10
    };
  ...
  const result = (0, _graph.graph)(nodes, edges, layers);
  return {
    ...result,
    size: {
      ...result.size,
