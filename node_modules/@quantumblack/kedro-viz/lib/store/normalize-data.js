...
exports.default = exports.createInitialPipelineState = void 0;
 ...
 */
exports.createInitialPipelineState = createInitialPipelineState;
const validateInput = data => {
  ...
  if (!Array.isArray(data.edges) || !Array.isArray(data.nodes)) {
    ...
    throw new Error('Invalid Kedro-Viz data input. Please ensure that your pipeline data includes arrays of nodes and edges');
  }
  return true;
};
...
const addPipeline = state => pipeline => {
  ...
  state.pipeline.name[id] = (0, _utils.prettifyName)(pipeline.name || '');
};
...
const addNode = state => node => {
  ...
  state.node.name[id] = (0, _utils.prettifyName)((0, _utils.stripNamespace)(node.name || ''));
  state.node.fullName[id] = node.name;
  state.node.type[id] = node.type;
  state.node.layer[id] = node.layer;
  state.node.pipelines[id] = node.pipelines ? (0, _utils.arrayToObject)(node.pipelines, () => true) : {};
  ...
  state.node.overview[id] = node.overview || [];
  state.node.documentation_url[id] = node.documentation_url || [];
};
...
const addTag = state => tag => {
  ...
  state.tag.name[id] = (0, _utils.prettifyName)(tag.name || '');
};
...
const getNodeTagsFiltersFromUrl = function (state, tagsQueryParam) {
  let allNodeTags = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
...
const updateStateWithFilters = (state, NodeTags) => {
  ...
  const typeQueryParams = search.get(_config.params.types);
  const tagQueryParams = search.get(_config.params.tags);
  const updatedStateWithTags = getNodeTagsFiltersFromUrl(state, tagQueryParams, NodeTags);
  const updatedStateWithTypes = getNodeTypesFromUrl(state, typeQueryParams);
  return {
    ...state,
    ...updatedStateWithTags,
...
const normalizeData = (data, expandAllPipelines) => {
  ...
  if (data.modular_pipelines) {
    ...
    state.modularPipeline.tree = (0, _utils.prettifyModularPipelineNames)(data.modular_pipelines);
...
var _default = exports.default = normalizeData;