"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.getTooltipPosition = void 0;
var _config = require("../../../config");
const tooltipMaxWidth = 300;
const tooltipLeftGap = 85;
const tooltipRightGap = 70;
const tooltipTopGap = 150;
const getTooltipPosition = (e, sidebarVisible) => {
  const xCoordsAdjustment = sidebarVisible ? 0 : _config.sidebarWidth.pipelineUI;
  const y = e.clientY - tooltipTopGap;
  let x, direction;
  if (window.innerWidth - e.clientX > tooltipMaxWidth) {
    x = e.clientX - _config.sidebarWidth.open - tooltipRightGap;
    direction = 'right';
  } else {
    x = e.clientX - _config.sidebarWidth.open - _config.sidebarWidth.open / 2 - tooltipLeftGap;
    direction = 'left';
  }
  x = x + xCoordsAdjustment;
  return {
    x,
    y,
    direction
  };
};
exports.getTooltipPosition = getTooltipPosition;