...
exports.thresholds = exports.rangeUnion = exports.rangeEqual = exports.range = exports.default = void 0;
...
const LazyList = _ref => {
  let {
    ...
    buffer = 0.5,
    onChange,
    container = element => element === null || element === void 0 ? void 0 : element.offsetParent
  } = _ref;
  ...
  const [range, setRange] = (0, _react.useState)([0, 0]);
  const rangeRef = (0, _react.useRef)([0, 0]);

  // List container element
  const listRef = (0, _react.useRef)();

  // Upper placeholder element
  const upperRef = (0, _react.useRef)();

  // Lower placeholder element
  const lowerRef = (0, _react.useRef)();

  // Height of a single item
  const itemHeight = (0, _react.useMemo)(() => height(0, 1), [height]);

  // Height of all items
  const totalHeight = (0, _react.useMemo)(() => height(0, total), [height, total]);

  // Height of items above the rendered range
  const upperHeight = (0, _react.useMemo)(() => height(0, range[0]), [height, range]);

  // Height of items below the rendered range
  const lowerHeight = (0, _react.useMemo)(() => height(range[1], total), [height, range, total]);
  ...
  (0, _react.useCallback)(() => {
    ...
    container(listRef.current), buffer, total, itemHeight);
  ...
  const observerOptions = (0, _react.useMemo)(() => ({
  ...
  (0, _react.useLayoutEffect)(() => requestUpdate(), [total, itemHeight, totalHeight, requestUpdate]);
  ...
  }), [active, range, total, listRef, upperRef, lowerRef, totalHeight, upperHeight, lowerHeight]);
 ...
 */
const range = (start, end, min, max) => [Math.max(Math.min(start, max), min), Math.max(Math.min(end, max), min)];
 ...
 */
exports.range = range;
const rangeUnion = (rangeA, rangeB) => [Math.min(rangeA[0], rangeB[0]), Math.max(rangeA[1], rangeB[1])];
 ...
 */
const thresholds = total => total === 0 ? [0] : [...Array.from({
  length: total
}, (_, i) => i / total), 1];
...
var _default = exports.default = LazyList;